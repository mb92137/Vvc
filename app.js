...
function drawCarDetailFX(){
  ctx.save();
  const t=performance.now();
  // metallic highlight and glass reflection on the player sedan
  ctx.globalAlpha=.18;ctx.fillStyle="#fff";
  ctx.beginPath();ctx.ellipse(player.x-6,player.y-22,7,25,-.22,0,Math.PI*2);ctx.fill();
  ctx.globalAlpha=.12;ctx.fillStyle="#bfe7ff";
  ctx.beginPath();ctx.roundRect(player.x-18,player.y-37,36,13,4);ctx.fill();
  // road contact shadow
  ctx.globalAlpha=.16;ctx.fillStyle="#000";
  ctx.beginPath();ctx.ellipse(player.x,player.y+29,28,7,0,0,Math.PI*2);ctx.fill();
  // subtle wheel motion highlights
  ctx.globalAlpha=.25;ctx.strokeStyle="#fff";ctx.lineWidth=1;
  for(const dx of [-16,16]){ctx.beginPath();ctx.arc(player.x+dx,player.y+20,5,0,t*.004% (Math.PI*2));ctx.stroke()}
  ctx.globalAlpha=1;ctx.restore();
}
function drawCarLightsFX(){
  ctx.save();
  const t=performance.now();
  // player headlamps and rear glow
  ctx.globalAlpha=.22;ctx.fillStyle="#fff7cf";
  for(const dx of [-11,11]){ctx.beginPath();ctx.arc(player.x+dx,player.y-40,3+Math.sin(t*.008)*.4,0,Math.PI*2);ctx.fill()}
  // enemy brake lights based on proximity
  if(typeof enemies!=="undefined"){
    ctx.globalAlpha=.18;ctx.fillStyle="#ff3030";
    for(const e of enemies){if(e.y>130&&e.y<520){for(const dx of [-9,9]){ctx.beginPath();ctx.arc(e.x+dx,e.y+28,2.5,0,Math.PI*2);ctx.fill()}}}
  }
  ctx.globalAlpha=1;ctx.restore();
}
function drawRainAndWetRoad(){
  ctx.save();
  const t=performance.now();
  const rain=(typeof speed==='number'&&speed>80)?Math.min(1,(speed-80)/120):0;
  if(rain>0){
    ctx.globalAlpha=.10+.12*rain;ctx.strokeStyle="#d8e7ff";ctx.lineWidth=1;
    for(let i=0;i<55;i++){const x=(i*83+t*.32)%W,y=(i*47+t*.62)%H;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x-3,y+13);ctx.stroke()}
  }
  // wet-road sheen
  ctx.globalAlpha=.07*rain;ctx.fillStyle="#b9d8ff";
  ctx.beginPath();ctx.moveTo(road.x+road.w*.34,190);ctx.lineTo(road.x+road.w*.66,190);ctx.lineTo(road.x+road.w*.92,620);ctx.lineTo(road.x+road.w*.08,620);ctx.closePath();ctx.fill();
  // headlight beams from player
  ctx.globalAlpha=.08;
  const beam=ctx.createLinearGradient(player.x,player.y-45,player.x,70);beam.addColorStop(0,"rgba(255,255,220,.35)");beam.addColorStop(1,"rgba(255,255,220,0)");ctx.fillStyle=beam;ctx.beginPath();ctx.moveTo(player.x-12,player.y-42);ctx.lineTo(player.x-75,75);ctx.lineTo(player.x+75,75);ctx.lineTo(player.x+12,player.y-42);ctx.closePath();ctx.fill();
  ctx.globalAlpha=1;ctx.restore();
}
function drawNextGenScene(){
  ctx.save();
  const t=performance.now();
  // layered atmospheric depth
  const horizon=155;
  const sky=ctx.createLinearGradient(0,0,0,horizon+180);
  sky.addColorStop(0,"#18243b");sky.addColorStop(.55,"#536b86");sky.addColorStop(1,"#b6a27e");
  ctx.globalAlpha=.16;ctx.fillStyle=sky;ctx.fillRect(0,0,W,horizon+190);
  // distant mountains / skyline silhouettes
  ctx.globalAlpha=.18;ctx.fillStyle="#202a32";ctx.beginPath();ctx.moveTo(0,horizon+35);
  for(let x=0;x<=W;x+=45){ctx.lineTo(x,horizon+15+Math.sin(x*.017)*24+Math.sin(x*.041)*10)}
  ctx.lineTo(W,horizon+100);ctx.lineTo(0,horizon+100);ctx.closePath();ctx.fill();
  // roadside light pools
  ctx.globalAlpha=.10;
  for(let i=0;i<6;i++){const x=road.x+(i%2?road.w-8:8),y=220+i*70;const g=ctx.createRadialGradient(x,y,2,x,y,55);g.addColorStop(0,"#fff");g.addColorStop(1,"rgba(255,255,255,0)");ctx.fillStyle=g;ctx.beginPath();ctx.arc(x,y,55,0,Math.PI*2);ctx.fill()}
  // subtle scanline-free film grain
  ctx.globalAlpha=.025;ctx.fillStyle="#fff";
  for(let i=0;i<45;i++){const x=(i*137+t*.01)%W,y=(i*71)%H;ctx.fillRect(x,y,1,1)}
  ctx.globalAlpha=1;ctx.restore();
}
